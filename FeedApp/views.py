...
@login_required # decerator - it a function that runs verification once
# prevent unauthorize actions to pages. People shoul log in to access this function

def profile(request): # this will control posting thins to webpage, getting things from webpage to our datbase
    profile= Profile.objects.filter(user=request.user) # user that currently logged in. "user" is the one of the attribut inb the profile table
    if not profile.exists():
        Profile.objects.create(user=request.user) # create a profile
    profile= Profile.objects.get(user=request.user) # get the actual profile of user
...
@login_required
def myfeed(request): # in my feed we would like to see all the post along comment nad likes
    comment_count_list=[]
    like_count_list=[]
    ...
    return render(request, 'FeedApp/myfeed.html', context)
       ...
       # if we have more than one we use filter, get is for only one
    # newst post on the top
...
def comments(request, post_id):
    if request.method == 'POST' and request.POST.get("btn1"): # check someone click the comment link and submit button was clicked
        comment = request.POST.get("comment") # we getting whatever text in the comment box
...
@login_required
def friends(request):
    #get the admin_profile and user profile  oto create the first relationship
    admin_profile = Profile.objects.get(user=1) # first person created the account is the admin
   ...
   # to get My Friends
    ...
    # if this is the first time to access the friends requests page, create the first relationship
